...
import { fetchMinesWins } from "../lib/api";
...
export default function LiveMinesWins() {
  const queryClient = useQueryClient();
  const ws = useRef<WebSocket | null>(null);

  const { data: liveWins = [] } = useQuery<Win[]>({
    queryKey: ["live-wins-mines"],
    queryFn: fetchMinesWins,
  });
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (msg.event === "liveWin" && msg.data.game === GameType.Mines) {
          const win: Win = {
            game: msg.data.game,
            user: msg.data.user,
          ...
          queryClient.setQueryData<Win[]>(["live-wins-mines"], (old = []) => {
