    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center px-4">
      <div className="bg-[#1C1C1C] text-white rounded-xl w-full max-w-[800px] p-6 border border-white/10 relative">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="bg-[#C8A2FF] text-black rounded-full px-4 py-2 font-semibold">
            1,250 Points
          </button>
        </div>
        ...
        {activeTab === 'daily' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {dailyPoints.map((entry) => (
              <div
                key={entry.day}
                className="w-full h-auto bg-[#212121] border border-white/10 rounded-[20px] px-6 py-4 flex flex-col justify-between items-center text-center"
              >
                ...
                {entry.status === 'claimed' ? (
                  ...
                  <button
                    onClick={() => {}}
                    className="w-full bg-[#C8A2FF] hover:bg-[#D5B3FF] text-black rounded-lg py-2 text-sm font-semibold"
                  >
                    Claim
                  </button>
                ) : (
