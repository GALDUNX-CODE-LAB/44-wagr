...
import { fetchPlinkoWins } from "../lib/api";
...
export default function LivePlinkoWins() {
  const queryClient = useQueryClient();
  const ws = useRef<WebSocket | null>(null);

  const { data: liveWins = [] } = useQuery<Win[]>({
    queryKey: ["live-wins-plinko"],
    queryFn: fetchPlinkoWins,
  });
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (msg.event === "liveWin" && msg.data.game === GameType.Plinko) {
          const win: Win = {
            game: msg.data.game,
            user: msg.data.user,
          ...
          queryClient.setQueryData<Win[]>(["live-wins-plinko"], (old = []) => {
