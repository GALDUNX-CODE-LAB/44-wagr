...
import { fetchRedlightWins } from "../lib/api";
...
export default function LiveRedlightWins() {
  const queryClient = useQueryClient();
  const ws = useRef<WebSocket | null>(null);

  const { data: liveWins = [] } = useQuery<Win[]>({
    queryKey: ["live-wins-redlight"],
    queryFn: async () => {
      const raw = await fetchRedlightWins();
      return (Array.isArray(raw) ? raw : []).map((item: any) => ({
        game: item.game,
        user: item.user,
        time: new Date(item.time).toLocaleTimeString(),
        bet: String(item.betAmount ?? item.bet ?? 0),
        multiplier: String(item.multiplier ?? 0),
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (msg.event === "liveWin" && msg.data.game === GameType.RedLight) {
          const win: Win = {
            game: msg.data.game,
            user: msg.data.user,
          ...
          queryClient.setQueryData<Win[]>(["live-wins-redlight"], (old = []) => {
