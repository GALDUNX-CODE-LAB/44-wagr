...
export default function LotteryCard({
  ...
  return (
    ...
    >
      {/* Profile Circle */}
      <div className="absolute -top-6 -left-5 w-[100px] h-[100px]  rounded-full overflow-hidden">
        <Image
      ...
      <div className="flex justify-end items-start mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">{card.totalBets}</span>
        </div>
      </div>

      {/* Lottery Name */}
      <div className="mt-12 mb-4 flex-grow">
        <p className="text-left text-base text-white/65">{card.name}</p>
      </div>

      {/* Price & Bet Count */}
      <div className="flex justify-between items-center mb-2">
        <span className="text-lg text-white">
          ${card.ticketPrice.toLocaleString()}
        </span>
      ...
      <p className="text-left text-sm text-white/60 mb-3">
        {card.isCompleted ? "Ended" : "Ends in"}
      </p>
