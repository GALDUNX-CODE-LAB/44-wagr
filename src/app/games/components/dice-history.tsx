...
import { fetchDiceGameHistory } from "../../../lib/api";
...
export default function DiceHistoryTable() {
  const { data: history } = useQuery({
    queryKey: ["dice-history"],
    queryFn: () => fetchDiceGameHistory(1),
    refetchInterval: 5000, // Refresh every 5 seconds
  });

  // Get latest 10 items from the history
  // Response structure: { data: bets[], total, game, page }
  const betsData = history?.data || [];
  const latestBets = betsData.slice(0, 10).map((bet: any) => ({
    ...bet,
    id: bet._id || bet.id, // Handle MongoDB _id
    game: bet.game || "Dice", // Add game name if not present
  }));
        ...
        <tbody>
          {latestBets.map((row: GameHistoryItem, idx: number) => (
            <tr key={row.id} className={`border-t border-white/5 ${idx % 2 === 0 ? "bg-[#1C1C1C]" : "bg-[#212121]"}`}>
              ...
              <td className="px-6 py-4 text-white/80">{row.multiplier.toFixed(2)}x</td>
              <td className="px-6 py-4">
                <div className="flex items-center gap-2">
                  <span className={`${row.payout > 0 ? "text-[#00ff5f]" : "text-white/80"}`}>
