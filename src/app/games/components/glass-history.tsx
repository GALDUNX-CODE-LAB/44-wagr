...
import { fetchGlassHistory } from "../../../lib/api";
...
export default function GlassHistoryTable() {
  const { data: history } = useQuery({
    queryKey: ["glass-history"],
    queryFn: () => fetchGlassHistory(1),
        ...
        <tbody>
          {latestBets.map((row: GameHistoryItem, idx: number) => (
            <tr key={row.id} className={`border-t border-white/5 ${idx % 2 === 0 ? "bg-[#1C1C1C]" : "bg-[#212121]"}`}>
              ...
              <td className="px-6 py-4 text-white/80">{(row.multiplier ?? 0).toFixed(2)}x</td>
              <td className="px-6 py-4">
